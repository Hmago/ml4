...
const DIAGRAM_TARGETS = [
  ...
  },
  {
    ...
    title: "The six DP parents, each with a worked miniature",
    rawBody: "Draw an identification poster titled 'THE SIX DP PARENTS' with six cards, each showing a tiny concrete example. Use large legible text and reproduce all numbers exactly. At the top a single wide gate box headed 'IS IT DP?' with two short lines: 'Is there a CHOICE at each step?' and 'Does it ask min, max, count, or possible?', then the line 'Both yes = it is DP'. One arrow leads down to six cards arranged in two rows of three. Each card has a coloured header, one short italic line beginning 'spot it:', and then a small worked miniature drawn as a picture. Card 1, teal, 'LINEAR - PICK OR SKIP', spot it: one array, decide at each element. Miniature: a row of five boxed numbers 2, 7, 9, 3, 1 with the 2, the 9 and the 1 circled, and the caption 'rob non-neighbours: 2 + 9 + 1 = 12'. Card 2, yellow, '0/1 KNAPSACK', spot it: items plus a budget, each item used ONCE. Miniature: a single horizontal strip of eight cells containing exactly these numbers in order: 0, 1, 1, 4, 5, 7, 8, 9, with a small index strip above reading 0 to 7 labelled 'bag space', and the final 9 circled in orange. Caption below: 'weights 1 3 4 5, bag of 7, best value 9'. Card 3, amber, 'UNBOUNDED KNAPSACK', spot it: items plus a budget, items REUSED. Miniature: a strip of seven cells with the numbers 0, 1, 2, 1, 1, 2, 2 and the caption 'coins 1 3 4 - to make 6 you need 2 coins'. Card 4, blue, 'LCS', spot it: TWO strings, or one string and its reverse. Miniature: the letters a b c d e above the letters a c e, with a, c and e joined by lines, captioned 'shared in order: ace, length 3'. Card 5, indigo, 'GRID', spot it: a 2D board with restricted movement. Miniature: a 3 by 3 grid of the numbers 1, 1, 1 on the top row, 1, 2, 3 on the middle row, 1, 3, 6 on the bottom row, with the 6 circled, captioned 'only right and down: 6 paths'. Card 6, orange, 'MCM - PARTITION', spot it: you CUT, BRACKET, BURST or MERGE a range. Miniature: a row of six cells with a dashed cut line down the middle, captioned 'try every cut, keep the cheapest'. Across the bottom, one banner: 'Learn these six cold - every other problem is one of them with ONE small change.' Flat modern educational poster style, distinct colour per card, miniatures small but readable, no photorealism, no code. Reproduce every number exactly as written; do not recompute or invent values."
  },
  {
    ...
    title: "Fibonacci of 5 solved four ways",
    rawBody: "Draw a four-panel worked example titled 'ONE PROBLEM, FOUR WAYS - Fibonacci of 5'. Use large legible text and real numbers exactly as given. PANEL 1, red header 'PLAIN RECURSION'. Draw the actual call tree for fib(5): root node 'fib(5)' splits into 'fib(4)' and 'fib(3)'. 'fib(4)' splits into 'fib(3)' and 'fib(2)'. That 'fib(3)' splits into 'fib(2)' and 'fib(1)'. The other 'fib(3)' splits into 'fib(2)' and 'fib(1)'. Draw every 'fib(2)' node filled in red. Beside the tree put a small tally box listing exactly: 'fib(3) computed 2 times', 'fib(2) computed 3 times', 'fib(1) computed 5 times'. Footer strip: '15 calls for an answer of 5'. PANEL 2, amber header 'ADD A MEMO'. Show the same tree but with every repeated node greyed out and crossed, and a curved arrow from the first 'fib(2)' into a small boxed array labelled 'memo'. Footer strip: 'each value computed once'. PANEL 3, light green header 'FILL A TABLE'. Show a single horizontal array of six cells with an index strip above reading 0, 1, 2, 3, 4, 5 and the cell values below reading exactly 0, 1, 1, 2, 3, 5. Shade the first two cells green and label them 'base case'. Draw a small arrow from cells 3 and 4 combining into cell 5, labelled 'add the two before it'. Footer strip: 'no recursion left'. PANEL 4, green header 'KEEP TWO NUMBERS'. Show only two circles labelled 'prev = 3' and 'curr = 5' with a small looping arrow between them. Footer strip: 'constant memory'. Join the four panels with thick arrows. Flat modern educational style, clean, numbers unmistakable, no photorealism, no code. Reproduce every number exactly as written; do not recompute or invent values."
  },
  {
    ...
    title: "The 0/1 knapsack table, filled in",
    rawBody: "Draw a single large worked example titled '0/1 KNAPSACK - the table, filled in'. Use large legible text and reproduce all numbers exactly. TOP STRIP: show four item cards left to right: 'ITEM 1  weight 1  value 1', 'ITEM 2  weight 3  value 4', 'ITEM 3  weight 4  value 5', 'ITEM 4  weight 5  value 7'. To their right draw a simple bag icon tagged 'capacity 7'. CENTRE: a clean grid, 5 rows by 8 columns, with a header row across the top reading 0, 1, 2, 3, 4, 5, 6, 7 titled 'capacity' and a header column down the left reading 'no items', 'item 1', 'item 2', 'item 3', 'item 4' titled 'items allowed'. Fill the grid with these exact values, row by row from top to bottom. Row 'no items': 0, 0, 0, 0, 0, 0, 0, 0. Row 'item 1': 0, 1, 1, 1, 1, 1, 1, 1. Row 'item 2': 0, 1, 1, 4, 5, 5, 5, 5. Row 'item 3': 0, 1, 1, 4, 5, 6, 6, 9. Row 'item 4': 0, 1, 1, 4, 5, 7, 8, 9. Shade the whole top row and the whole first column light green and label them once: 'no items or no space - always 0'. Circle the very last cell, the 9 in the bottom right corner, in bold orange and tag it 'the answer: 9'. DERIVATION CALLOUT: highlight the cell holding 9 in the row 'item 3' (the last column of that row). Draw two arrows into it from the row above, 'item 2'. One arrow comes from the cell in that row holding 5 in the last column, labelled 'LEAVE item 3, keep 5'. The other arrow comes from the cell in that row holding 4, labelled 'TAKE item 3, its value 5 plus the 4 already there, makes 9'. Add a small badge between them reading 'keep the bigger: 9'. BOTTOM BANNER in red: 'Going backwards along the row is what stops an item being used twice.' Flat modern educational style, grid lines crisp, every digit clearly readable, no photorealism, no code. Reproduce every number exactly as written; do not recompute or invent values."
  },
  {
    id: "dsa31_dp_lcs",
    chapterFile: "content/31c_dynamic_programming.md",
    ...
    title: "The LCS table for abcde against ace, plus the reverse-string trick",
    rawBody: "Draw a two-part worked example titled 'LCS - fill the table, then walk it backwards'. Use large legible text and reproduce all letters and numbers exactly. PART A on the left, titled 'abcde  vs  ace'. Draw a grid of 6 rows by 4 columns. The header row across the top reads: blank, a, c, e. The header column down the left reads: blank, a, b, c, d, e. Fill the body with these exact rows, top to bottom: blank row: 0, 0, 0, 0. Row a: 0, 1, 1, 1. Row b: 0, 1, 1, 1. Row c: 0, 1, 2, 2. Row d: 0, 1, 2, 2. Row e: 0, 1, 2, 3. Shade the blank top row and blank first column light green, labelled once 'an empty string shares nothing'. Highlight in orange the three cells that form the answer path: the 1 in row a under column a, the 2 in row c under column c, and the 3 in row e under column e. Join them with a bold dashed diagonal line running from the bottom-right 3 up to the top-left 1, with arrows pointing up-left. Beside the path write: 'letters match - step diagonally and collect the letter'. Below the grid show three letter tiles reading a, c, e and the caption 'read backwards, then reverse: the answer is ace, length 3'. PART B on the right, a highlighted panel titled 'THE HIDDEN SECOND STRING'. Show six letter tiles reading a, g, b, c, b, a. Below, a downward arrow captioned 'LCS needs two strings - you were given one'. Below that, six tiles reading a, b, c, b, g, a captioned 'the same string, reversed'. Below that an arrow into a small grid icon, then a result box of five tiles reading a, b, c, b, a captioned 'the longest palindrome inside it, length 5'. Close with one line: 'A palindrome matches its own reverse.' Flat modern educational style, grid crisp, letters and digits unmistakable, no photorealism, no code. Reproduce every number and letter exactly as written; do not recompute or invent values."
  },
  {
    id: "dsa31_dp_mcm",
    chapterFile: "content/31c_dynamic_programming.md",
    ...
    title: "Bracketing changes the cost of a matrix chain",
    rawBody: "Draw a worked example titled 'WHERE YOU PUT THE BRACKETS CHANGES THE COST'. Use large legible text and reproduce all numbers exactly. TOP: three matrix blocks drawn as labelled rectangles in a row, sized roughly in proportion: 'A  10 by 30', 'B  30 by 5', 'C  5 by 60'. Beside them a small note: 'the answer is the same either way - the work is not'. MIDDLE, two contrasting panels side by side. LEFT PANEL, green, headed '(A B) C'. Two steps listed: step 1 'multiply A by B, cost 10 x 30 x 5 = 1500, leaves a 10 by 5 block', step 2 'multiply that by C, cost 10 x 5 x 60 = 3000'. A large total box reads 'TOTAL 4500' with a green tick. RIGHT PANEL, red, headed 'A (B C)'. Two steps listed: step 1 'multiply B by C, cost 30 x 5 x 60 = 9000, leaves a 30 by 60 block', step 2 'multiply A by that, cost 10 x 30 x 60 = 18000'. A large total box reads 'TOTAL 27000' with a red cross. Between the two panels a bold badge reading 'SIX TIMES THE WORK'. BOTTOM, headed 'SO WHERE DOES THE BRACKET GO?'. Draw a horizontal row of six boxed cells; highlight the leftmost and label it 'i', highlight the rightmost and label it 'j', and draw a bold dashed vertical line between two middle cells labelled 'k'. A curly brace under the left part reads 'solve this side', a curly brace under the right part reads 'solve that side', and a small box beneath reads 'plus the cost of joining them'. Under it, three faded copies of the same row with the dashed line in three different positions, captioned 'try every k and keep the cheapest'. BOTTOM BANNER in red: 'Solve the shortest stretches first, not left to right.' Flat modern educational style, clean, every number readable, no photorealism, no code. Reproduce every number exactly as written; do not recompute or invent values."
  },
  {
    ...
    title: "0/1 versus unbounded knapsack on the same input",
    rawBody: "Draw a side-by-side comparison titled 'ONE INDEX IS THE WHOLE DIFFERENCE'. Both halves use the SAME items: weights 1, 3, 4, 5 with values 1, 4, 5, 7 and capacity 7. Use large legible text and reproduce all numbers exactly. LEFT HALF, blue header '0/1 KNAPSACK - each item once'. Draw a grid of 5 rows by 8 columns, header row across the top reading 0, 1, 2, 3, 4, 5, 6, 7, header column down the left reading 'none', 'item 1', 'item 2', 'item 3', 'item 4'. Fill it with these exact rows top to bottom: 'none': 0, 0, 0, 0, 0, 0, 0, 0. 'item 1': 0, 1, 1, 1, 1, 1, 1, 1. 'item 2': 0, 1, 1, 4, 5, 5, 5, 5. 'item 3': 0, 1, 1, 4, 5, 6, 6, 9. 'item 4': 0, 1, 1, 4, 5, 7, 8, 9. Under this grid a caption: 'after taking an item, look at the row ABOVE - the item is used up'. Draw one clear arrow from a cell in row 'item 2' diagonally up-left into row 'item 3', to show the lookup crossing rows. RIGHT HALF, amber header 'UNBOUNDED KNAPSACK - items reusable'. Draw an identical 5 by 8 grid with the same headers, filled with these exact rows top to bottom: 'none': 0, 0, 0, 0, 0, 0, 0, 0. 'item 1': 0, 1, 2, 3, 4, 5, 6, 7. 'item 2': 0, 1, 2, 4, 5, 6, 8, 9. 'item 3': 0, 1, 2, 4, 5, 6, 8, 9. 'item 4': 0, 1, 2, 4, 5, 7, 8, 9. Under this grid a caption: 'after taking an item, stay on the SAME row - you may take it again'. Draw one clear arrow travelling horizontally leftward WITHIN a single row, to show the lookup staying on its row. BETWEEN THE TWO GRIDS, a tall narrow highlight panel with a downward-pointing finger or marker and the short text: 'Look at row item 1. On the left it is all ones - one item, used once. On the right it counts 1 2 3 4 5 6 7 - the same 1kg item taken over and over.' BOTTOM BANNER in red: 'Same code. Previous row means once. Same row means many times.' Flat modern educational style, the two grids clearly aligned for comparison, every digit readable, no photorealism, no code. Reproduce every number exactly as written; do not recompute or invent values."
  },
  // ---------------------------------------------------------------------------
  // Chapter 31 (DSA: Foundations & Search) — pattern-recognition concept diagrams.
  // Prefix dsa31_ (no _dp_ infix, which belongs to chapter 31c).
  // ---------------------------------------------------------------------------
  {
    id: "dsa31_map",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![Nine trigger phrases, each pointing at the one template that solves it](diagrams/dsa31_map_ai.png)",
    svgBase: "dsa31_map",
    title: "Nine trigger phrases, each pointing at the one template that solves it",
    rawBody: "Draw a large two-column lookup chart titled 'SEE THIS, REACH FOR THIS'. Use large legible text and reproduce every phrase exactly as written. The LEFT column is headed 'what the question says' and the RIGHT column is headed 'the template that solves it'. Draw nine rows; each row is a rounded left tile joined by a thick arrow to a rounded right tile, and each right tile carries a small dark complexity badge. Row 1: left 'sorted array, find a pair' arrow right 'TWO POINTERS, converging' badge 'O(n)'. Row 2: left 'remove, move or compact in place' arrow right 'TWO POINTERS, same direction' badge 'O(n)'. Row 3: left 'sum or count over a range' arrow right 'PREFIX SUM' badge 'O(n)'. Row 4: left 'have I seen this before' arrow right 'HASHMAP or HASHSET' badge 'O(n)'. Row 5: left 'longest or shortest run of items next to each other' arrow right 'SLIDING WINDOW' badge 'O(n)'. Row 6: left 'next greater, next smaller, or matching brackets' arrow right 'MONOTONIC STACK' badge 'O(n)'. Row 7: left 'sorted input, or smallest value that still works' arrow right 'BINARY SEARCH' badge 'O(log n)'. Row 8: left 'top K, or a running median' arrow right 'HEAP' badge 'O(n log k)'. Row 9: left 'prefix, autocomplete or dictionary' arrow right 'TRIE' badge 'O(word length)'. Colour-code the nine arrows in nine distinct hues. Across the bottom draw a full-width banner in a contrasting colour reading: 'Name the template before you write a line of code.' Clean flat modern educational style, generous spacing, every phrase clearly readable, no photorealism, no code."
  },
  {
    id: "dsa31_bigo",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![The complexity ladder, and the input size at which each rung stops working](diagrams/dsa31_bigo_ai.png)",
    svgBase: "dsa31_bigo",
    title: "The complexity ladder, and the input size at which each rung stops working",
    rawBody: "Draw a two-part illustration about algorithm cost. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. LEFT PANEL titled 'THE LADDER'. Draw seven steps of a staircase climbing from bottom-left to top-right, coloured from green at the bottom through amber to red at the top. Label the steps from bottom to top: 'O(1)  instant lookup', 'O(log n)  halve the problem', 'O(n)  one pass', 'O(n log n)  sort', 'O(n squared)  nested loops', 'O(2 to the n)  try every subset', 'O(n factorial)  try every order'. Beside the bottom three steps write in green 'fine at a million'; beside the top three write in red 'fine only when n is tiny'. RIGHT PANEL titled 'WILL IT BE TOO SLOW?'. Draw a horizontal budget bar labelled at its right end 'about 100 million simple steps per second'. Under it draw a table of six rows with the headers 'biggest n' and 'slowest shape you can still use'. The rows, top to bottom, read exactly: '10' and 'O(n factorial)'; '20' and 'O(2 to the n)'; '500' and 'O(n cubed)'; '10,000' and 'O(n squared)'; '1,000,000' and 'O(n log n)'; '100,000,000' and 'O(n)'. Beside the table draw a small red warning badge with the text 'n = 10,000 with nested loops is 100 million steps, right at the edge; n = 100,000 is 10 billion and will time out'. BOTTOM BANNER: 'Read the constraint first. It names your complexity, and your complexity names your pattern.' Clean flat modern educational style, every number readable, no photorealism, no code."
  },
  {
    id: "dsa31_twoptr",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![Two pointers, two shapes: squeeze from both ends, or a writer chased by a reader](diagrams/dsa31_twoptr_ai.png)",
    svgBase: "dsa31_twoptr",
    title: "Two pointers, two shapes: squeeze from both ends, or a writer chased by a reader",
    rawBody: "Draw a side-by-side comparison of the two two-pointer shapes. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. LEFT PANEL, blue, titled 'CONVERGING - they walk toward each other'. Subtitle 'sorted list, find two numbers that add to 9'. Draw three stacked rows of four boxed numbers reading 2, 7, 11, 15 in every row. In row one place a green arrow labelled 'left' under the 2 and a green arrow labelled 'right' under the 15, with the note to the right '2 + 15 = 17, too big, pull the right pointer in'. In row two keep 'left' under the 2 and move 'right' under the 11, with the note '2 + 11 = 13, still too big, pull in again'. In row three keep 'left' under the 2 and place 'right' under the 7, with the note in green '2 + 7 = 9, found it' and a tick. Add a small caption under the panel: 'sorted order is what makes this safe - moving right can only ever shrink the sum'. RIGHT PANEL, amber, titled 'SAME DIRECTION - a slow writer and a fast reader'. Subtitle 'squeeze the duplicates out of 1, 1, 2, 2, 3'. Draw three stacked rows of five boxed numbers. Row one reads 1, 1, 2, 2, 3 with a blue marker labelled 'write' under the first box and an orange marker labelled 'read' under the second box, noted 'same value, the reader just walks on'. Row two reads 1, 2, 2, 2, 3 with 'write' under the second box and 'read' under the third box, noted 'new value, copy it back and step the writer'. Row three reads 1, 2, 3, 2, 3 with 'write' under the third box and 'read' past the end, noted in green 'three unique values, the first three boxes are the answer'. Shade the first three boxes of row three green and the last two grey, labelled 'ignored'. BOTTOM BANNER spanning both panels: 'Converging shrinks a search. Same-direction rewrites an array in place - and both cost one pass.' Clean flat modern educational style, arrows and markers unmistakable, no photorealism, no code."
  },
  {
    id: "dsa31_prefix",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![A prefix array turns every range sum into a single subtraction](diagrams/dsa31_prefix_ai.png)",
    svgBase: "dsa31_prefix",
    title: "A prefix array turns every range sum into a single subtraction",
    rawBody: "Draw a worked example titled 'ADD ONCE, SUBTRACT FOREVER'. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. TOP ROW: six boxed numbers reading 3, 1, 4, 1, 5, 9, labelled above 'the array' and indexed underneath 0, 1, 2, 3, 4, 5. SECOND ROW: seven boxed numbers reading 0, 3, 4, 8, 9, 14, 23, labelled above 'running total, one box longer' and indexed underneath 0, 1, 2, 3, 4, 5, 6. Draw thin grey arrows from each array box up into the running-total box to its right, showing the total accumulating. Place a small note beside the leading 0 reading 'always start with 0, it means nothing added yet'. MIDDLE SECTION titled 'now any range is free'. Highlight in orange the array boxes at index 1, 2 and 3 (the values 1, 4 and 1) with a brace beneath reading 'wanted: 1 + 4 + 1 = 6'. Below, show a large subtraction laid out as two highlighted running-total boxes: the box holding 9 labelled 'everything up to the end of the range' minus the box holding 3 labelled 'everything before the range starts', with a bold equals sign and a green result box reading 6, and beside it a tick with the words 'one subtraction, no matter how long the range'. BOTTOM SECTION, split in two. LEFT, red, headed 'without it': a loop icon inside a loop icon with the caption 'add the same numbers again for every question - O(n squared)'. RIGHT, green, headed 'with it': a single straight arrow with the caption 'build once in O(n), then answer every question in O(1)'. BOTTOM BANNER: 'Any question about a stretch of a list is a subtraction of two running totals.' Clean flat modern educational style, every digit readable, no photorealism, no code."
  },
  {
    id: "dsa31_hashmap",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![Inside a hashmap: a key becomes a bucket number, and collisions form a chain](diagrams/dsa31_hashmap_ai.png)",
    svgBase: "dsa31_hashmap",
    title: "Inside a hashmap: a key becomes a bucket number, and collisions form a chain",
    rawBody: "Draw a left-to-right pipeline titled 'WHY A LOOKUP IS INSTANT'. Use large legible text and reproduce every word and number exactly as written. STAGE 1, a rounded tile reading 'apple'. An arrow labelled 'hash the key' leads to STAGE 2, a tile reading '987234'. An arrow labelled 'squeeze it into the table, 987234 wrapped onto 16 slots' leads to STAGE 3, a vertical column of eight numbered slots labelled 0 to 7, drawn as a tall bucket rack. Slot 2 is highlighted and holds a small card reading 'apple, 5'. Draw a chain link from that card to a second card reading 'grape, 3', with a red callout beside it reading 'COLLISION - two keys landed in the same slot, so they queue up in a short list'. Slot 3 holds one card reading 'banana, 7'. The other slots are empty and greyed. Caption under the rack: 'go straight to one slot - never scan the whole table'. RIGHT PANEL, stacked in two parts. TOP part headed 'when the rack gets crowded': a small gauge at three quarters full with the text 'more than about three quarters full, the rack doubles and every key is re-filed - rare, and the cost spreads out to roughly nothing per insert'. BOTTOM part headed 'when one chain gets long': a short chain of eight cards turning into a small balanced tree shape, with the text 'a chain longer than eight becomes a tree, so the worst case is O(log n) instead of O(n)'. BOTTOM BANNER: 'Average O(1). It is the first thing to try whenever a nested loop is searching for something you have already seen.' Clean flat modern educational style, no photorealism, no code."
  },
  {
    id: "dsa31_fastslow",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![Fast and slow pointers: the runner analogy, the meeting point, and the cycle entrance](diagrams/dsa31_fastslow_ai.png)",
    svgBase: "dsa31_fastslow",
    title: "Fast and slow pointers: the runner analogy, the meeting point, and the cycle entrance",
    rawBody: "Draw a three-part illustration about two pointers moving at different speeds. Use large legible text and reproduce every label exactly as written. PANEL A, left, titled 'THE IDEA'. Draw an oval running track with two small runner icons on it, one labelled 'slow, one step' in blue and one labelled 'fast, two steps' in orange, with a caption beneath: 'on a straight road the fast runner simply finishes first; on a loop, the fast runner must eventually lap the slow one - so if they ever stand on the same spot, there is a loop'. PANEL B, middle, titled 'FINDING THE LOOP'. Draw six circular nodes labelled 1, 2, 3, 4, 5, 6 connected by arrows 1 to 2 to 3 to 4 to 5 to 6, and a curved arrow from 6 back to 3 so that nodes 3, 4, 5, 6 form a loop. Below the node picture draw a small five-row trace table with the columns 'step', 'slow', 'fast' and these exact rows: '0', '1', '1'; '1', '2', '3'; '2', '3', '5'; '3', '4', '3'; '4', '5', '5'. Highlight the final row in green and label it 'both on node 5 - a loop exists'. PANEL C, right, titled 'WHERE DOES THE LOOP START?'. Draw the same six-node picture. Place a green marker labelled 'restart at the head, node 1' and an orange marker labelled 'leave the other at node 5'. Add the caption 'now move BOTH one step at a time' and draw two short dashed paths meeting at node 3, which is circled in green and labelled 'the loop entrance'. Beside it a small note: 'two steps each, and they meet exactly at the entrance'. BOTTOM BANNER: 'Same trick, different question: stop the fast pointer at the end instead, and the slow pointer is standing on the middle node.' Clean flat modern educational style, node numbers unmistakable, no photorealism, no code."
  },
  {
    id: "dsa31_monostack",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![A monotonic stack is a queue of people waiting for someone taller](diagrams/dsa31_monostack_ai.png)",
    svgBase: "dsa31_monostack",
    title: "A monotonic stack is a queue of people waiting for someone taller",
    rawBody: "Draw an illustration titled 'EVERYONE IS WAITING FOR SOMEONE TALLER'. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. TOP STRIP: six bars of increasing-then-varying height drawn as simple standing figures or columns, labelled underneath with their heights 2, 1, 4, 3, 5 and indexed 0, 1, 2, 3, 4. Caption: 'for each person, who is the first TALLER person to their right?'. MIDDLE SECTION, a five-step horizontal strip showing the waiting queue after each arrival, each step drawn as a small vertical stack of tiles with the newest on top. Step 1, arrival 2: stack shows a tile reading 2, note 'nobody to compare with, it waits'. Step 2, arrival 1: stack shows tiles 2 then 1 above it, note '1 is shorter, so it simply joins the queue'. Step 3, arrival 4: stack shows a single tile reading 4, note in green '4 is taller than 1 and taller than 2 - both get their answer, 4, and leave'. Step 4, arrival 3: stack shows 4 then 3 above it, note '3 is shorter, it waits'. Step 5, arrival 5: stack shows a single tile reading 5, note in green '5 is taller than 3 and taller than 4 - both get their answer, 5, and leave'. Draw small green tick badges where an answer is produced. BOTTOM SECTION titled 'the answers', a row of five result boxes reading 4, 4, 5, 5, -1, aligned under the original heights 2, 1, 4, 3, 5, with the last one in grey labelled 'still waiting when the parade ends - nobody taller exists'. BOTTOM BANNER: 'Each person joins the queue once and leaves once, so the whole thing costs O(n), not O(n squared).' Clean flat modern educational style, stack tiles clearly ordered, no photorealism, no code."
  },
  {
    id: "dsa31_binsearch",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![Binary search is not about arrays, it is about finding the false-to-true border](diagrams/dsa31_binsearch_ai.png)",
    svgBase: "dsa31_binsearch",
    title: "Binary search is not about arrays, it is about finding the false-to-true border",
    rawBody: "Draw a two-panel illustration. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. LEFT PANEL, blue, titled 'THE BORDER'. Draw a row of six boxed numbers reading 1, 3, 3, 3, 5, 7, indexed 0 to 5 underneath. Above each box place a letter tile: N, Y, Y, Y, Y, Y, with a small key reading 'is this value at least 3?'. Shade the N region red and the Y region green, and draw a thick vertical dashed border between index 0 and index 1, labelled with a large arrow 'the first Y - this is what binary search actually returns'. Caption beneath: 'once the answers go no, no, no, yes, yes, yes and never flip back, you can always throw away half'. RIGHT PANEL, amber, titled 'THE SAME BORDER, BUT THERE IS NO ARRAY'. Subtitle 'banana piles 3, 6, 7, 11 - eat them all within 8 hours - what is the slowest speed that still works?'. Draw a horizontal ruler of candidate speeds marked 1 through 11. Above it place N tiles over speeds 1, 2 and 3 and Y tiles over speeds 4 through 11, with the same red and green shading and a thick dashed border between 3 and 4, labelled 'answer, speed 4'. Beside the ruler show two small check cards. Card one, red, headed 'try speed 3': the lines 'pile 3 takes 1 hour', 'pile 6 takes 2 hours', 'pile 7 takes 3 hours', 'pile 11 takes 4 hours', 'total 10 hours, too slow'. Card two, green, headed 'try speed 4': the lines 'pile 3 takes 1 hour', 'pile 6 takes 2 hours', 'pile 7 takes 2 hours', 'pile 11 takes 3 hours', 'total 8 hours, just fits'. BOTTOM BANNER: 'If you can answer yes or no about a candidate answer, and yes never turns back into no, you can binary search the answer itself.' Clean flat modern educational style, the red-to-green border unmistakable in both panels, no photorealism, no code."
  },
  {
    id: "dsa31_window",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![The sliding window: the right edge always moves, the left edge only moves to fix a broken window](diagrams/dsa31_window_ai.png)",
    svgBase: "dsa31_window",
    title: "The sliding window: the right edge always moves, the left edge only moves to fix a broken window",
    rawBody: "Draw a worked example titled 'GROW ON THE RIGHT, SHRINK ON THE LEFT'. Use large legible text and reproduce every letter exactly as written. Subtitle: 'longest stretch with no repeated letter, in a b c a b c b b'. Draw six stacked rows; every row shows the same eight letter tiles reading a, b, c, a, b, c, b, b, indexed 0 to 7 underneath the first row only. In each row draw a coloured translucent rectangle over the tiles that are currently inside the window, with a green arrow labelled 'left' under its first tile and a blue arrow labelled 'right' under its last tile, and a note to the right of the row. Row 1: window covers index 0 only, note 'take a - window is a, size 1'. Row 2: window covers indexes 0 to 1, note 'take b - window is a b, size 2'. Row 3: window covers indexes 0 to 2, note in green 'take c - window is a b c, size 3, best so far'. Row 4: window covers indexes 0 to 3 drawn in red with a warning badge, note in red 'take a - but a is already inside, the window is broken'. Row 5: window covers indexes 1 to 3, note 'push the left edge past the old a - window is b c a, size 3, valid again'. Row 6: window covers indexes 2 to 4, note 'carry on - the answer stays 3'. At the bottom draw a small two-box diagram: a box reading 'right edge' with a long arrow reading 'moves every single step, n times in total' and a box reading 'left edge' with a shorter arrow reading 'only moves forward, never back, so at most n times in total', joined by a result badge reading 'two pointers, one pass, O(n)'. BOTTOM BANNER: 'Ask only two questions: what makes the window invalid, and what do I record when it is valid?' Clean flat modern educational style, window rectangle clearly visible in each row, no photorealism, no code."
  },
  {
    id: "dsa31_treewalk",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![One tree, four walks: preorder, inorder, postorder and level order](diagrams/dsa31_treewalk_ai.png)",
    svgBase: "dsa31_treewalk",
    title: "One tree, four walks: preorder, inorder, postorder and level order",
    rawBody: "Draw an illustration titled 'ONE TREE, FOUR WALKS'. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. CENTRE: a small binary tree drawn four times, once in each of four quadrants, always with the same shape - a root circle labelled 1, its left child labelled 2 and its right child labelled 3, and node 2 having a left child labelled 4 and a right child labelled 5. TOP-LEFT QUADRANT headed 'PREORDER - visit me, then my children', with a numbered path drawn over the tree and a result strip of five tiles reading 1, 2, 4, 5, 3, plus the note 'use it to copy or print a tree from the top down'. TOP-RIGHT QUADRANT headed 'INORDER - left, me, right', result strip reading 4, 2, 5, 1, 3, plus a green highlighted note 'on a binary search tree this comes out sorted - that single fact solves a whole family of problems'. BOTTOM-LEFT QUADRANT headed 'POSTORDER - children first, me last', result strip reading 4, 5, 2, 3, 1, plus the note 'use it when a node needs its children answers first, like heights, sums and deletes'. BOTTOM-RIGHT QUADRANT headed 'LEVEL ORDER - one row at a time, using a queue', with three horizontal bands drawn across the tree and a result showing three grouped strips: a strip reading 1, then a strip reading 2, 3, then a strip reading 4, 5, plus the note 'use it for shortest path in an unweighted tree, or anything phrased per level'. BOTTOM BANNER: 'Depth-first for anything about a path or a subtree. Breadth-first for anything about a level or the nearest thing.' Clean flat modern educational style, all four trees identical in shape, digits unmistakable, no photorealism, no code."
  },
  {
    id: "dsa31_heap",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![Top K with a small min-heap: the smallest of the winners guards the door](diagrams/dsa31_heap_ai.png)",
    svgBase: "dsa31_heap",
    title: "Top K with a small min-heap: the smallest of the winners guards the door",
    rawBody: "Draw a worked example titled 'KEEP THE BEST THREE WITHOUT SORTING ANYTHING'. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. TOP: a stream of seven numbered cards arriving left to right reading 5, 1, 9, 3, 7, 2, 8, labelled 'numbers arriving one at a time'. MIDDLE: a row of seven small panels, one per arrival, each showing a tiny box holding up to three values with the SMALLEST value drawn at the front in red and labelled 'the doorkeeper'. Panel 1, arrival 5: box holds 5, note 'room to spare'. Panel 2, arrival 1: box holds 1, 5, note 'room to spare'. Panel 3, arrival 9: box holds 1, 5, 9, note 'now full'. Panel 4, arrival 3: box holds 3, 5, 9, note in green '3 beats the doorkeeper 1, so 1 is dropped'. Panel 5, arrival 7: box holds 5, 7, 9, note in green '7 beats the doorkeeper 3, so 3 is dropped'. Panel 6, arrival 2: box still holds 5, 7, 9 with a red cross over the arriving 2, note in red '2 loses to the doorkeeper 5, so it is rejected instantly'. Panel 7, arrival 8: box holds 7, 8, 9, note in green '8 beats the doorkeeper 5, so 5 is dropped'. BOTTOM LEFT: a green result badge reading 'the top three are 7, 8, 9'. BOTTOM RIGHT: a two-row comparison table with the rows 'sort everything first' with the cost 'O(n log n), and it stores all n' and 'the small heap' with the cost 'O(n log k), and it stores only k'. Add a small note: 'to keep the LARGEST K, hold a heap whose smallest value guards the door - it feels backwards, and it is the part people get wrong'. BOTTOM BANNER: 'Whenever the question says top K, K largest, K closest, or running median, reach for a heap.' Clean flat modern educational style, digits unmistakable, no photorealism, no code."
  },
  {
    id: "dsa31_trie",
    chapterFile: "content/31_dsa_foundations.md",
    promptKind: "concept",
    existingImageLine: "![A trie stores each shared prefix once, so autocomplete is a walk plus a sweep](diagrams/dsa31_trie_ai.png)",
    svgBase: "dsa31_trie",
    title: "A trie stores each shared prefix once, so autocomplete is a walk plus a sweep",
    rawBody: "Draw a two-panel illustration. Use large legible text and reproduce every letter exactly as written. LEFT PANEL titled 'THE SHAPE'. Draw a tree of small letter circles growing downward from an empty root circle. From the root, one branch to a circle reading a, then down to a circle reading p. From that p, one branch down to another circle reading p which is drawn with a thick green ring, and from that a further circle reading l then a circle reading e also drawn with a thick green ring. Back at the first p, a second branch leads to a circle reading e then a circle reading x with a thick green ring. Separately from the root, a branch to a circle reading b then a then t with a thick green ring. Add a key box reading 'a green ring means a complete word ends here'. Label the shared a-p path with a highlighted band reading 'app, apple and apex all share this path - stored once, not three times'. Under the tree list the stored words: app, apple, apex, bat. RIGHT PANEL titled 'AUTOCOMPLETE IN TWO MOVES'. Step 1, a badge reading 'the user has typed a p': draw a bold arrow tracing the root to a to p path, captioned 'walk down two letters - cost is the length of what was typed, nothing else'. Step 2, a badge reading 'then sweep everything below': draw a shaded cone covering the whole subtree under that p, with three result tiles beside it reading app, apple, apex. Below, a red comparison card headed 'the hashmap way' with the text 'check every word in the dictionary against the prefix - fine for ten words, hopeless for ten million'. BOTTOM BANNER: 'Hashmap for exact lookups. Trie the moment the question says prefix, autocomplete, or dictionary.' Clean flat modern educational style, letters unmistakable, no photorealism, no code."
  },
  // ---------------------------------------------------------------------------
  // Chapter 31b (DSA: Graphs) — concept diagrams.
  // ---------------------------------------------------------------------------
  {
    id: "dsa31b_map",
    chapterFile: "content/31b_dsa_graphs.md",
    promptKind: "concept",
    existingImageLine: "![Seven graph triggers, each pointing at the algorithm that answers it](diagrams/dsa31b_map_ai.png)",
    svgBase: "dsa31b_map",
    title: "Seven graph triggers, each pointing at the algorithm that answers it",
    rawBody: "Draw a two-column lookup chart titled 'GRAPH QUESTIONS, AND WHAT ANSWERS THEM'. Use large legible text and reproduce every phrase exactly as written. LEFT column headed 'what the question asks', RIGHT column headed 'the algorithm', each right tile carrying a small dark cost badge. Draw seven rows joined by thick coloured arrows. Row 1: 'fewest steps, all moves cost the same' arrow 'BFS' badge 'O(V + E)'. Row 2: 'does a path exist, or explore everything' arrow 'DFS' badge 'O(V + E)'. Row 3: 'cheapest route, edge costs differ' arrow 'DIJKSTRA' badge 'O((V+E) log V)'. Row 4: 'order tasks that depend on each other' arrow 'TOPOLOGICAL SORT' badge 'O(V + E)'. Row 5: 'are these two connected, asked over and over' arrow 'UNION-FIND' badge 'almost O(1)'. Row 6: 'is there a loop' arrow 'THREE-COLOUR DFS, or UNION-FIND' badge 'O(V + E)'. Row 7: 'islands, regions, or a maze on a grid' arrow 'BFS or DFS ON THE GRID' badge 'O(rows x cols)'. Below the rows add a wide highlighted strip titled 'FIRST, FIND THE DISGUISE' containing four small cards reading: 'a grid - every cell is a node, its neighbours are edges', 'a word list - every word is a node, one-letter changes are edges', 'course prerequisites - every course is a node, each rule is a directed edge', 'a state puzzle - every arrangement is a node, every legal move is an edge'. BOTTOM BANNER: 'Most graph problems never mention the word graph. Name the nodes and the edges first.' Clean flat modern educational style, generous spacing, no photorealism, no code."
  },
  {
    id: "dsa31b_bfsdfs",
    chapterFile: "content/31b_dsa_graphs.md",
    promptKind: "concept",
    existingImageLine: "![BFS spreads in rings, DFS dives down one path — the only difference is queue versus stack](diagrams/dsa31b_bfsdfs_ai.png)",
    svgBase: "dsa31b_bfsdfs",
    title: "BFS spreads in rings, DFS dives down one path",
    rawBody: "Draw a side-by-side comparison titled 'ONE LINE OF DIFFERENCE, TWO COMPLETELY DIFFERENT WALKS'. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. Both halves use the SAME small graph, drawn identically: four circular nodes labelled 0, 1, 2 and 3, with lines joining 0 to 1, 0 to 2, 1 to 2, and 2 to 3. LEFT HALF, blue, headed 'BFS - a QUEUE, oldest out first'. Over the graph draw two faint concentric rings centred on node 0: the inner ring passing through nodes 1 and 2, the outer ring passing through node 3. Label the rings 'one step away' and 'two steps away'. Beneath, a four-row trace table with columns 'queue', 'take out', 'put in' and these exact rows: '[0]', '0', '1, 2'; '[1, 2]', '1', 'nothing new'; '[2]', '2', '3'; '[3]', '3', 'nothing new'. Under the table a green result strip reading 'visit order 0, 1, 2, 3' and a caption 'because it finishes a whole ring before starting the next, the first time BFS reaches a node is by the shortest route'. RIGHT HALF, amber, headed 'DFS - a STACK, newest out first'. Over the graph draw one bold winding arrow diving 0 to 1 to 2 to 3, with a dashed return arrow curling back labelled 'backtrack'. Beneath, a four-row trace table with columns 'call', 'at node', 'goes to' and these exact rows: 'dfs(0)', '0', '1'; 'dfs(1)', '1', '2'; 'dfs(2)', '2', '3'; 'dfs(3)', '3', 'nothing new - back up'. Under the table an amber result strip reading 'visit order 0, 1, 2, 3' and a caption 'it commits to one path to the very end, so the first route it finds is NOT necessarily the shortest'. BOTTOM BANNER: 'Queue means rings and shortest paths. Stack means one deep path, backtracking, cycles and ordering.' Clean flat modern educational style, both graphs identical in layout, no photorealism, no code."
  },
  {
    id: "dsa31b_dijkstra",
    chapterFile: "content/31b_dsa_graphs.md",
    promptKind: "concept",
    existingImageLine: "![Dijkstra settles the nearest unsettled node, then relaxes its edges](diagrams/dsa31b_dijkstra_ai.png)",
    svgBase: "dsa31b_dijkstra",
    title: "Dijkstra settles the nearest unsettled node, then relaxes its edges",
    rawBody: "Draw a worked example titled 'ALWAYS SETTLE THE NEAREST ONE LEFT'. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. TOP: a graph of four circular nodes labelled A, B, C and D joined by labelled edges: A to B costs 1, A to C costs 4, B to C costs 2, B to D costs 5, C to D costs 1. MIDDLE: four step panels left to right, each showing the same graph small with the settled nodes filled solid green and a distance label beside every node. Panel 1 headed 'settle A, distance 0': labels read A 0, B 1, C 4, D infinity, with a note 'A reaches B for 1 and C for 4'. Panel 2 headed 'settle B, distance 1': labels read A 0, B 1, C 3, D 6, with an orange highlight on C and the note 'going through B reaches C for 1 + 2 = 3, cheaper than the direct 4 - this is a RELAXATION'. Panel 3 headed 'settle C, distance 3': labels read A 0, B 1, C 3, D 4, with an orange highlight on D and the note 'through C, D costs 3 + 1 = 4, cheaper than 6'. Panel 4 headed 'settle D, distance 4': all four nodes solid green, labels A 0, B 1, C 3, D 4, note 'nothing left to settle'. BOTTOM LEFT: a green result card reading 'shortest distances from A: A 0, B 1, C 3, D 4'. BOTTOM RIGHT: a red warning card headed 'WHY NEGATIVE EDGES BREAK IT' containing the text 'once a node is settled Dijkstra never revisits it - a later negative edge could have made it cheaper, so the answer would be wrong. Use Bellman-Ford instead.' BOTTOM BANNER: 'A priority queue just answers one question over and over: which unsettled node is nearest?' Clean flat modern educational style, every distance label readable, no photorealism, no code."
  },
  {
    id: "dsa31b_topo",
    chapterFile: "content/31b_dsa_graphs.md",
    promptKind: "concept",
    existingImageLine: "![Topological sort peels off whatever currently depends on nothing](diagrams/dsa31b_topo_ai.png)",
    svgBase: "dsa31b_topo",
    title: "Topological sort peels off whatever currently depends on nothing",
    rawBody: "Draw a worked example titled 'KEEP TAKING WHATEVER DEPENDS ON NOTHING'. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. TOP: a directed graph of four circular nodes labelled 0, 1, 2 and 3, with arrows drawn 3 to 1, 3 to 2, 1 to 0 and 2 to 0. Beside each node write a small counter badge labelled 'waiting on' holding these exact values: node 3 shows 0, node 1 shows 1, node 2 shows 1, node 0 shows 2. Caption: 'the counter is how many arrows point INTO a node - how many things it is still waiting for'. MIDDLE: four step panels left to right, each showing the ready queue, the node taken, and the updated counters. Panel 1: queue '[3]', take '3', note 'its arrows are removed, so 1 and 2 each drop to 0', counters now 1 shows 0, 2 shows 0, 0 shows 2. Panel 2: queue '[1, 2]', take '1', note '0 drops to 1', counters 0 shows 1. Panel 3: queue '[2]', take '2', note '0 drops to 0 and joins the queue', counters 0 shows 0. Panel 4: queue '[0]', take '0', note 'queue empty, finished'. BOTTOM LEFT: a green result strip of four ordered tiles reading 3, 1, 2, 0, captioned 'a valid order - 3 then 2 then 1 then 0 would be equally valid'. BOTTOM RIGHT: a red card headed 'AND IT DETECTS CYCLES FOR FREE' with a small three-node ring drawn inside and the text 'if the queue empties before every node has been taken, the leftovers are all waiting on each other - that is a cycle, and no valid order exists'. BOTTOM BANNER: 'Course prerequisites, build order, task scheduling - all the same peeling.' Clean flat modern educational style, counters unmistakable, no photorealism, no code."
  },
  {
    id: "dsa31b_unionfind",
    chapterFile: "content/31b_dsa_graphs.md",
    promptKind: "concept",
    existingImageLine: "![Union-Find: every group is a tree, and path compression flattens it on the way back](diagrams/dsa31b_unionfind_ai.png)",
    svgBase: "dsa31b_unionfind",
    title: "Union-Find: every group is a tree, and path compression flattens it",
    rawBody: "Draw a three-part illustration titled 'WHO IS YOUR LEADER?'. Use large legible text and reproduce every number exactly as written. PART A, left, headed 'THE IDEA': draw two small clusters of circles, one cluster of three nodes all with arrows pointing up to a node wearing a small crown, the other cluster of two nodes likewise. Caption: 'every group keeps one leader at the top. Two items are in the same group if, and only if, following the arrows up lands you on the same leader.' PART B, middle, headed 'MERGING - always attach the SHORTER tree under the taller one': show a before and after. BEFORE: a two-level tree with leader 0 and a one-node tree with leader 3, separated by a plus sign. AFTER: leader 0 at the top with the node 3 now hanging beneath it. Beside it a red crossed-out mini panel showing the taller tree attached under the shorter one, labelled 'do it the wrong way round and the trees grow tall and slow'. PART C, right, headed 'PATH COMPRESSION - the part that makes it fast': show a BEFORE picture of a tall chain, four circles in a vertical line labelled top to bottom 0, 1, 2, 3 with each arrow pointing up to the one above, and a marker at node 3 labelled 'find(3) walks all the way up - slow'. Then a bold downward arrow labelled 'on the way back, re-point every node it touched straight at the leader'. Then an AFTER picture: node 0 wearing the crown with nodes 1, 2 and 3 all hanging directly off it in one flat row, labelled 'every future find on these nodes is one hop'. BOTTOM BANNER: 'Union by rank keeps the tree short; path compression flattens what is left. Together, near-constant time per question.' Clean flat modern educational style, arrows clearly directional, no photorealism, no code."
  },
  {
    id: "dsa31b_cycle",
    chapterFile: "content/31b_dsa_graphs.md",
    promptKind: "concept",
    existingImageLine: "![Three-colour DFS: grey means the node is on the path you are standing in](diagrams/dsa31b_cycle_ai.png)",
    svgBase: "dsa31b_cycle",
    title: "Three-colour DFS: grey means the node is on the path you are standing in",
    rawBody: "Draw an illustration titled 'A LOOP IS AN ARROW BACK INTO THE PATH YOU ARE STANDING IN'. Use large legible text and reproduce every label exactly as written. TOP: a colour key of three circles - a white circle labelled 'WHITE - not visited yet', a grey circle labelled 'GREY - I am inside this node right now, it is on my current path', a black circle labelled 'BLACK - finished, I have already backed out of it'. MIDDLE: a directed graph of four circular nodes labelled 0, 1, 2 and 3 with arrows 0 to 1, 1 to 2, 2 to 3, and a curved arrow 3 back to 1 drawn in bold red. Show four small stacked snapshots of this graph to the right, each recolouring the nodes as the search proceeds: snapshot 1 with node 0 grey and the rest white; snapshot 2 with 0 and 1 grey; snapshot 3 with 0, 1 and 2 grey; snapshot 4 with 0, 1, 2 and 3 grey and the red arrow into node 1 highlighted, carrying a red badge reading 'node 1 is GREY - it is still on my path - so this arrow closes a loop'. BOTTOM LEFT, a contrasting panel headed 'BLACK IS NOT A CYCLE': draw a small graph with arrows 0 to 1, 0 to 2 and 1 to 2, with node 2 coloured black and a green tick, captioned 'reaching a finished node just means two routes lead to the same place - a diamond, not a loop. This is the mistake people make.' BOTTOM RIGHT, a panel headed 'UNDIRECTED GRAPHS ARE EASIER': three circles joined in a triangle with the text 'just union the ends of every edge - if the two ends already share a leader, that edge closes a loop'. BOTTOM BANNER: 'Directed: three-colour DFS. Undirected: Union-Find.' Clean flat modern educational style, the grey and black distinction unmistakable, no photorealism, no code."
  },
  {
    id: "dsa31b_grid",
    chapterFile: "content/31b_dsa_graphs.md",
    promptKind: "concept",
    existingImageLine: "![A grid is a graph: flood fill counts regions, multi-source BFS spreads from everywhere at once](diagrams/dsa31b_grid_ai.png)",
    svgBase: "dsa31b_grid",
    title: "A grid is a graph: flood fill counts regions, multi-source BFS spreads at once",
    rawBody: "Draw a two-panel illustration. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. TOP STRIP, full width, headed 'A GRID IS ALREADY A GRAPH': draw a 3 by 3 grid of squares with the centre square highlighted, and four bold arrows leaving it up, down, left and right into its neighbours, each arrow labelled 'edge'. Beside it a small card reading 'every cell is a node, its four neighbours are its edges - you never need to build an adjacency list, just step in four directions'. LEFT PANEL, green, headed 'FLOOD FILL - count the regions'. Draw a 4 by 5 grid of squares filled with these exact values, row by row from the top: row 1: 1, 1, 0, 0, 0; row 2: 1, 1, 0, 0, 0; row 3: 0, 0, 1, 0, 0; row 4: 0, 0, 0, 1, 1. Shade the three groups of 1s in three different colours and circle each group, labelling them 'island 1', 'island 2' and 'island 3'. Below, three short steps: 'walk every cell', 'when you hit an unvisited 1, add one to the count', 'then sink its whole connected blob so it is never counted again'. A green result badge reads '3 islands'. RIGHT PANEL, amber, headed 'MULTI-SOURCE BFS - everything spreads at the same time'. Draw a 3 by 3 grid filled with these exact values, row by row: row 1: 2, 1, 1; row 2: 1, 1, 0; row 3: 0, 1, 1. Label the 2 as 'already rotten' and the 0s as 'empty'. Beside it draw three small copies of the same grid showing the spread, labelled 'minute 1', 'minute 2' and so on, with newly rotted cells highlighted each time, and a final result badge reading '4 minutes'. Add a key card: 'put EVERY starting cell into the queue before the loop begins - then one queue level equals one minute for all of them at once'. BOTTOM BANNER: 'Counting regions is DFS. Distance or time is BFS. Many starting points is multi-source BFS.' Clean flat modern educational style, grid digits unmistakable, no photorealism, no code."
  },
  // ---------------------------------------------------------------------------
  // Chapter 31d (DSA: Advanced Patterns & ML Coding) — concept diagrams.
  // ---------------------------------------------------------------------------
  {
    id: "dsa31d_map",
    chapterFile: "content/31d_dsa_advanced_ml_coding.md",
    promptKind: "concept",
    existingImageLine: "![The advanced-pattern picker: eight triggers and the tool each one names](diagrams/dsa31d_map_ai.png)",
    svgBase: "dsa31d_map",
    title: "The advanced-pattern picker: eight triggers and the tool each one names",
    rawBody: "Draw a two-column lookup chart titled 'THE ADVANCED SHELF - WHAT EACH TOOL IS FOR'. Use large legible text and reproduce every phrase exactly as written. LEFT column headed 'when the question says', RIGHT column headed 'reach for', each right tile with a small dark cost badge. Eight rows joined by thick coloured arrows. Row 1: 'list every combination, subset or arrangement' arrow 'BACKTRACKING' badge 'O(2 to the n) or O(n factorial)'. Row 2: 'take the best option now and never look back' arrow 'GREEDY' badge 'O(n log n)'. Row 3: 'find the odd one out, or count bits, with no extra memory' arrow 'BIT MANIPULATION' badge 'O(n), O(1) space'. Row 4: 'merge, overlap or schedule ranges' arrow 'SORT THE INTERVALS, THEN SWEEP' badge 'O(n log n)'. Row 5: 'range sums or minimums WITH updates' arrow 'SEGMENT TREE or FENWICK TREE' badge 'O(log n) each'. Row 6: 'shortest path, but some edges are negative' arrow 'BELLMAN-FORD' badge 'O(V x E)'. Row 7: 'shortest path between EVERY pair' arrow 'FLOYD-WARSHALL' badge 'O(V cubed)'. Row 8: 'maximum or minimum inside a sliding window' arrow 'MONOTONIC DEQUE' badge 'O(n)'. BOTTOM STRIP, a highlighted warning card titled 'BEFORE YOU REACH UP HERE': the text 'most interview problems are solved by the basics - two pointers, hashmaps, windows, BFS. These tools are for when a constraint rules the basics out. Say WHY the simple tool fails before you use a complicated one.' BOTTOM BANNER: 'Name the constraint that forces the tool.' Clean flat modern educational style, generous spacing, no photorealism, no code."
  },
  {
    id: "dsa31d_backtrack",
    chapterFile: "content/31d_dsa_advanced_ml_coding.md",
    promptKind: "concept",
    existingImageLine: "![Choose, explore, unchoose — and the one line that separates subsets, permutations and combinations](diagrams/dsa31d_backtrack_ai.png)",
    svgBase: "dsa31d_backtrack",
    title: "Choose, explore, unchoose — and the one line that separates the big three",
    rawBody: "Draw a two-part illustration. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. TOP PART, headed 'THE ONLY LOOP YOU NEED': draw three connected round badges in a cycle - a green badge reading '1. CHOOSE - add it to the current list', a blue badge reading '2. EXPLORE - recurse one level deeper', an amber badge reading '3. UNCHOOSE - remove it again' - joined by curved arrows into a loop. Beside the cycle a red note card reads 'the UNCHOOSE step is what people forget - without it the next branch inherits your leftovers'. BOTTOM PART, three panels side by side, each showing a small decision tree over the values 1 and 2. LEFT PANEL headed 'SUBSETS - take it or leave it', tree drawn as: a root labelled empty brackets, splitting into a left branch labelled 'take 1' leading to a node reading 1, and a right branch labelled 'skip 1' leading to a node reading empty brackets; each of those splits again on 'take 2' and 'skip 2', giving four leaves reading '1 2', '1', '2' and empty brackets. Note beneath: 'EVERY node counts as an answer, not just the leaves - 4 subsets'. MIDDLE PANEL headed 'PERMUTATIONS - every element, every order', tree drawn as: a root splitting into 'pick 1' and 'pick 2'; the left child then has only the option 'pick 2' giving a leaf reading '1 2'; the right child has only 'pick 1' giving a leaf reading '2 1'. Note beneath: 'a used-list blocks repeats, so only the LEAVES count - 2 orderings'. RIGHT PANEL headed 'COMBINATIONS - choose 2 from 1, 2, 3', tree drawn from a root with three branches: from 1 down to leaves reading '1 2' and '1 3'; from 2 down to a leaf reading '2 3'; from 3 down to a crossed-out grey dead end. Note beneath: 'a start index forbids looking backwards, so 2 1 can never appear - 3 combinations'. Add a highlighted strip across the bottom of the three panels reading: 'subsets pass i + 1 and record every node; permutations pass a used-list and record the leaves; combinations pass a start index.' BOTTOM BANNER: 'Same three steps every time. Only the rule about what you are allowed to pick next changes.' Clean flat modern educational style, trees clearly drawn, no photorealism, no code."
  },
  {
    id: "dsa31d_bits",
    chapterFile: "content/31d_dsa_advanced_ml_coding.md",
    promptKind: "concept",
    existingImageLine: "![The five bit tricks that actually come up, each shown on a real number](diagrams/dsa31d_bits_ai.png)",
    svgBase: "dsa31d_bits",
    title: "The five bit tricks that actually come up, each shown on a real number",
    rawBody: "Draw a grid of five worked cards titled 'BIT TRICKS, SHOWN ON REAL NUMBERS'. Use large legible text and reproduce every digit exactly as written; do not recompute or invent values. Every card shows its numbers as rows of small square boxes holding 0 or 1, with the decimal value beside them. CARD 1, headed 'XOR CANCELS PAIRS': three short rows reading '4 = 100', '4 = 100', 'result = 000 = 0', then a green summary strip reading 'so XOR everything together and the only number without a partner survives - that is how you find the single odd one out using no extra memory'. CARD 2, headed 'CLEAR THE LOWEST 1 - n AND (n minus 1)': rows reading '12 = 1100', '11 = 1011', 'AND = 1000 = 8', with the changed bit circled, and a summary reading 'repeat until zero and the number of repeats is the number of 1 bits'. CARD 3, headed 'ISOLATE THE LOWEST 1 - n AND minus n': rows reading '12 = 1100', 'minus 12 = ...0100 in the low bits', 'AND = 0100 = 4', and a summary reading 'this single value is what a Fenwick tree jumps by'. CARD 4, headed 'IS IT A POWER OF TWO?': a green example with rows '8 = 1000', '7 = 0111', 'AND = 0000, so YES' and a red example with rows '12 = 1100', '11 = 1011', 'AND = 1000, not zero, so NO', plus a summary reading 'a power of two has exactly one 1 bit, so clearing it leaves nothing'. CARD 5, headed 'READ, SET, CLEAR, FLIP ONE BIT': four short lines reading 'read bit i: shift right by i, then AND 1', 'set bit i: OR with 1 shifted left by i', 'clear bit i: AND with the flip of 1 shifted left by i', 'flip bit i: XOR with 1 shifted left by i'. BOTTOM BANNER in red: 'In Java, use the unsigned right shift for negative numbers - the ordinary right shift copies the sign bit and loops forever.' Clean flat modern educational style, every 0 and 1 unmistakable, no photorealism, no code."
  },
  {
    id: "dsa31d_intervals",
    chapterFile: "content/31d_dsa_advanced_ml_coding.md",
    promptKind: "concept",
    existingImageLine: "![Intervals: sort by start to merge, sort by end to keep the most](diagrams/dsa31d_intervals_ai.png)",
    svgBase: "dsa31d_intervals",
    title: "Intervals: sort by start to merge, sort by end to keep the most",
    rawBody: "Draw a two-panel illustration titled 'WHICH END YOU SORT BY DECIDES WHAT YOU GET'. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. Both panels draw intervals as horizontal bars above a shared numbered timeline running 0 to 10. LEFT PANEL, blue, headed 'SORT BY START - to MERGE overlaps'. Show three bars stacked and labelled with their ranges: a bar from 1 to 3, a bar from 2 to 6, and a bar from 8 to 10. Draw a bracket joining the first two bars labelled 'they touch, so they become one', and show a result row underneath with two merged bars reading '1 to 6' and '8 to 10'. Add a rule card: 'walk left to right - if the next bar starts before the current one ends, stretch the current end to the larger of the two; otherwise start a new bar'. LEFT PANEL BOTTOM, a small red note: 'the merged end must be the LARGER of the two ends - a short interval swallowed inside a long one must not shrink it'. RIGHT PANEL, green, headed 'SORT BY END - to KEEP THE MOST'. Show four bars labelled with their ranges: 1 to 2, 2 to 3, 1 to 3, and 3 to 4, arranged in order of their end values. Tick the bars 1 to 2, 2 to 3 and 3 to 4 in green, and cross out the bar 1 to 3 in red with the note 'it overlaps something already kept'. Add a green result badge reading 'keep 3, remove 1'. Add a rule card: 'the interval that ENDS EARLIEST leaves the most room for everything after it - so always keep that one and drop anything that clashes with it'. BOTTOM BANNER: 'Merging asks where things overlap, so sort by start. Scheduling asks how many fit, so sort by end.' Clean flat modern educational style, timeline numbers unmistakable, no photorealism, no code."
  },
  {
    id: "dsa31d_segtree",
    chapterFile: "content/31d_dsa_advanced_ml_coding.md",
    promptKind: "concept",
    existingImageLine: "![Why a range query needs a tree: prefix sums die the moment values change](diagrams/dsa31d_segtree_ai.png)",
    svgBase: "dsa31d_segtree",
    title: "Why a range query needs a tree: prefix sums die the moment values change",
    rawBody: "Draw a three-part illustration. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. PART A, top, headed 'THE PROBLEM': two small cards side by side. Left card, green, reading 'values never change: a running-total array answers any range in one subtraction - O(1)'. Right card, red, reading 'one value changes: every running total after it is now wrong, and rebuilding costs O(n). Do that on every update and the whole thing collapses.' Between them a bold arrow labelled 'so we need a structure that survives updates'. PART B, middle, headed 'SEGMENT TREE - every node owns a stretch'. Draw a binary tree over the array 1, 3, 5, 7. The four leaves read 1, 3, 5 and 7, each labelled with the range it covers. The two middle nodes read 4 and 12, labelled 'covers 1 to 3' and 'covers 5 to 7'. The root reads 16, labelled 'covers everything'. Draw a bold orange path from the leaf holding 3 up through the node 4 to the root 16, labelled 'change one leaf and only its ancestors need fixing - that is log n nodes, not n'. Beside the tree, a small card reading 'a range query breaks the asked-for stretch into a handful of whole nodes and adds them up'. PART C, bottom, headed 'FENWICK TREE - the same power, a quarter of the code'. Draw a single row of eight numbered slots labelled 1 to 8. Above them draw brackets showing the stretch each slot is responsible for: slot 1 covers just itself, slot 2 covers slots 1 to 2, slot 4 covers slots 1 to 4, slot 8 covers slots 1 to 8, while slots 3, 5, 6 and 7 cover shorter stretches. Caption: 'how far back a slot reaches is given by its lowest set bit - which is why Fenwick trees are written with bit tricks'. Add a comparison strip with two rows: 'Segment tree - sums, minimums, maximums, anything you can combine; more code' and 'Fenwick tree - sums and running totals only; far less code'. BOTTOM BANNER: 'Read-only ranges? Use running totals. Ranges plus updates? That is when you climb the tree.' Clean flat modern educational style, every number readable, no photorealism, no code."
  },
  {
    id: "dsa31d_shortestpaths",
    chapterFile: "content/31d_dsa_advanced_ml_coding.md",
    promptKind: "concept",
    existingImageLine: "![Three shortest-path algorithms, and the exact constraint that forces each one](diagrams/dsa31d_shortestpaths_ai.png)",
    svgBase: "dsa31d_shortestpaths",
    title: "Three shortest-path algorithms, and the constraint that forces each one",
    rawBody: "Draw a three-column comparison titled 'WHICH SHORTEST-PATH ALGORITHM, AND WHY'. Use large legible text and reproduce every label exactly as written. Each column is a tall card with a coloured header, a small graph sketch, a bullet list and a cost badge. COLUMN 1, blue, headed 'DIJKSTRA': sketch a four-node graph with positive edge numbers and one node filled green labelled 'settled - never revisited'. Bullets: 'one starting point', 'every edge cost must be zero or more', 'greedily settles the nearest node left'. Badge 'O((V + E) log V)'. Footer strip in red: 'a negative edge breaks it, because a settled node is never reconsidered'. COLUMN 2, amber, headed 'BELLMAN-FORD': sketch the same four nodes with one edge drawn in red carrying a minus sign. Bullets: 'one starting point', 'negative edges are allowed', 'just relaxes EVERY edge, over and over, V minus 1 times'. Badge 'O(V x E)'. Footer strip in green: 'run one extra round - if any distance still improves, there is a negative cycle, and shortest paths do not exist'. COLUMN 3, purple, headed 'FLOYD-WARSHALL': sketch a small four-by-four table of numbers labelled 'distance from every node to every node'. Bullets: 'EVERY pair at once, not one source', 'negative edges allowed, negative cycles not', 'asks one question repeatedly: is going via k cheaper?'. Badge 'O(V cubed)'. Footer strip: 'only practical for small graphs, roughly a few hundred nodes'. BOTTOM STRIP, a highlighted card headed 'THE LOOP ORDER PEOPLE GET WRONG' containing three stacked labels reading 'k on the OUTSIDE - the stopover', 'then i - the start', 'then j - the destination', with the explanation 'k must be outermost because each round allows one more stopover to be used by everything else. Put k inside and you are asking the question before the answers it depends on exist.' BOTTOM BANNER: 'Non-negative and one source? Dijkstra. Negative edges? Bellman-Ford. Every pair? Floyd-Warshall.' Clean flat modern educational style, no photorealism, no code."
  },
  {
    id: "dsa31d_deque",
    chapterFile: "content/31d_dsa_advanced_ml_coding.md",
    promptKind: "concept",
    existingImageLine: "![The monotonic deque throws away every value that can never be the maximum again](diagrams/dsa31d_deque_ai.png)",
    svgBase: "dsa31d_deque",
    title: "The monotonic deque throws away every value that can never win again",
    rawBody: "Draw a worked example titled 'IF SOMETHING BIGGER ARRIVES LATER, THE SMALLER ONE IS FINISHED'. Use large legible text and reproduce every number exactly as written; do not recompute or invent values. TOP: a row of eight numbered boxes reading 1, 3, -1, -3, 5, 3, 6, 7, indexed 0 to 7 underneath, with a caption 'find the biggest number in every window of 3'. MIDDLE, a key insight card in amber reading: 'when a new value arrives, every smaller value already waiting is finished forever - it is both smaller AND older, so it can never be the maximum of any future window. Throw them away.' BELOW THAT, six small step panels left to right. Each panel shows the current window highlighted over the eight boxes, a small horizontal deque drawn as a row of tiles with its front marked, and the answer produced. Panel 1: window covers 1, 3, -1; deque tiles read 3, -1; answer 3; note '3 arrived and knocked out 1'. Panel 2: window covers 3, -1, -3; deque tiles read 3, -1, -3; answer 3. Panel 3: window covers -1, -3, 5; deque tiles read 5; answer 5; note '5 knocked out everything - all of them were smaller and older'. Panel 4: window covers -3, 5, 3; deque tiles read 5, 3; answer 5. Panel 5: window covers 5, 3, 6; deque tiles read 6; answer 6. Panel 6: window covers 3, 6, 7; deque tiles read 7; answer 7. BOTTOM LEFT: a green result strip of six tiles reading 3, 3, 5, 5, 6, 7. BOTTOM RIGHT: two small cards - one red reading 'a heap also works, but removing the value that just slid out of the window costs O(log n) or needs lazy deletion' and one green reading 'the deque is O(n) overall - each value is pushed once and popped once'. Add a note card: 'also drop the front whenever its index has slid out of the window - that is the second check people forget'. BOTTOM BANNER: 'Front of the deque is always the current maximum. Everything behind it is a smaller, younger backup.' Clean flat modern educational style, every number readable, no photorealism, no code."
  },
];
